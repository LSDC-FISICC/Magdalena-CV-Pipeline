...
static const std::vector<std::string> CAMERAS_TOPICS {
    ...
    "/camera4/camera4/color/image_raw",
    "/camera5/camera5/color/image_raw",
};
...
cameraNode::cameraNode() : Node("camera_process_node"), m_cameraTopics(CAMERAS_TOPICS){
    ...
    for (const auto& topic : m_cameraTopics) {
        ...
        cv::namedWindow("Debug Preprocess" + topic, cv::WINDOW_AUTOSIZE);
    ...
    m_timer = this->create_wall_timer(
        std::chrono::milliseconds(50), // 15 Hz same as the cameras
        ...
        for(const auto& topic : m_cameraTopics){
            if(m_imageBuffer.count(topic) && m_imageBuffer[topic]!= nullptr){
                ...
                try{
                    ...
                    cv::imshow("Debug Preprocess" + topic, imgFloat);
                    cv::waitKey(1);
