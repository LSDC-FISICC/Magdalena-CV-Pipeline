...
InferenceNode_v2::InferenceNode_v2() : Node("Yolo_InferenceNode_v2"){
    ...
    this->declare_parameter<std::string>("engine_path", "./src/run_model_cpp_node/best.engine");
...
void InferenceNode_v2::imageCallback(const sensor_msgs::msg::Image::SharedPtr msg){
    ...
    try{
        ...
        for(i; i<results.size();  ++i){
            ...
            RCLCPP_INFO(this->get_logger(), "Topic: %s -> Pred: %d , %s,  (Conf: %.2f), batch size de : %d", 
                    camera_name.c_str(), results[i].predictedClass, classe_name.c_str(), results[i].confidence, batch_camera_name.size() );
            }
